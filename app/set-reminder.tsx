...
export default function TestScreen() {
  ...
  const routeParams = useLocalSearchParams<{
    ...
    reminderToEditNote?: string;
    reminderToEditTime?: string;
    ocurrence?: string;
    reminderEditPrayerId?: string;
  ...
  useFocusEffect(
    ...
    useCallback(() => {
      ...
      if (routeParams.reminderToEditTitle && routeParams.type !== "Add") {
        ...
        if (routeParams.ocurrence && routeParams.ocurrence !== "None") {
          setIsRepeat(true);
  ...
  const serializeReminder = (
    reminder: Reminder
  ): Omit<Reminder, "time"> & { time: string } => {
    const timeValue =
      reminder.time instanceof Date
        ? reminder.time.toISOString()
        : typeof reminder.time === "string"
        ? reminder.time
        : new Date(reminder.time).toISOString();
    return {
      ...reminder,
      time: timeValue,
    };
  ...
  const handleEditReminder = async () => {
    ...
    const newReminderObj = {
      id: uuid.v4(),
      message: newReminder,
      prayer_id: Array.isArray(routeParams.reminderEditPrayerId)
        ? routeParams.reminderEditPrayerId[0]
        : routeParams.reminderEditPrayerId || "",

      note: newNote,
      time: combinedDate,
    };

    if (routeParams.reminderIdentifier) {
  ...
  // iOS handlers
  const handleIOSDateConfirm = (date: Date) => {
  ...
  const handleIOSTimeConfirm = (time: Date) => {
